...
const els = {
  ...
  lofiTracksInput: $("#lofiTracksInput"),
...
function lofiTrackUrls(item) {
  const raw = Array.isArray(item?.trackUrls)
    ? item.trackUrls
    : String(item?.trackUrls || "").split(/\r?\n/);

  const unique = [];
  const seen = new Set();

  for (const entry of raw) {
    const url = normalizeSpotifyUrl(entry, "track");
    if (!url) continue;

    const id = spotifyEntityId(url);
...
function sortedLofiPlaylists() {
  return [...state.lofiPlaylists].sort((a, b) => {
    ...
    const campaignCompare = (ca?.title || "").localeCompare(
      cb?.title || "",
      "pt"
    );

    if (campaignCompare) return campaignCompare;
...
function selectedLofiPlaylist() {
  return state.lofiPlaylists.find(
    item => item.id === state.selectedLofiPlaylistId
  ) || null;
}
...
function ensureLofiSelection() {
  ...
  if (!exists) {
    ...
    state.lofiTrackIndex = -1;
    state.lofiTrackMode = false;
    state.lofiLoop = false;
  }
...
function lofiArtwork(item) {
  ...
  return (
    safeHttpUrl(item?.coverUrl) ||
    safeHttpUrl(campaign?.coverUrl) ||
    ""
  );
}
...
function lofiBackground(item) {
  ...
  return (
    safeHttpUrl(item?.backgroundUrl) ||
    safeHttpUrl(item?.coverUrl) ||
    safeHttpUrl(campaign?.coverUrl) ||
    ""
  );
...
function renderLofiPlaybackState() {
  ...
  els.lofiPlayBtn.textContent = state.lofiPlaying ? "Ⅱ" : "▶";
  els.lofiPlayBtn.classList.toggle("playing", state.lofiPlaying);
  els.lofiLoopBtn.classList.toggle("active", state.lofiLoop);
  els.lofiNavBars.classList.toggle("playing", state.lofiPlaying);

  els.lofiModeLabel.textContent = state.lofiTrackMode
    ? "TRACK MODE"
    : "PLAYLIST MODE";

  els.lofiPlaylistModeBtn.classList.toggle(
    "hidden",
    !state.lofiTrackMode
  );
}

function setLofiStatus(message) {
  if (els.lofiSpotifyStatus) {
    els.lofiSpotifyStatus.textContent = message;
  }
}

function loadLofiPlaylistEntity({ autoplay = false } = {}) {
  const item = selectedLofiPlaylist();
  const url = normalizeSpotifyUrl(item?.spotifyUrl, "playlist");
  ...
  if (state.spotifyController) {
    state.spotifyController.loadEntity(url, false, 0);
...
function loadLofiTrack(index, { autoplay = true } = {}) {
  const item = selectedLofiPlaylist();
  const tracks = lofiTrackUrls(item);

  if (!tracks.length || !state.spotifyController) {
    showToast("LOFI // Adiciona links das músicas no Admin.");
    return;
  }

  const normalizedIndex =
    ((Number(index) || 0) % tracks.length + tracks.length) % tracks.length;

  state.lofiTrackIndex = normalizedIndex;
  state.lofiTrackMode = true;

  state.spotifyController.loadEntity(
    tracks[normalizedIndex],
    false,
    0
  );
...
function lofiNext() {
  const tracks = lofiTrackUrls(selectedLofiPlaylist());

  if (!tracks.length) {
    showToast("LOFI // Regista as músicas desta playlist no Admin.");
    return;
  }

  const nextIndex =
    state.lofiTrackMode && state.lofiTrackIndex >= 0
      ? state.lofiTrackIndex + 1
      : 0;

  loadLofiTrack(nextIndex);
}

function lofiPrevious() {
  const tracks = lofiTrackUrls(selectedLofiPlaylist());
...
function syncLofiTrackIndexFromUri(uri) {
  const id = spotifyEntityId(uri);
  if (!id) return;

  const tracks = lofiTrackUrls(selectedLofiPlaylist());
  const index = tracks.findIndex(track => spotifyEntityId(track) === id);

  if (index >= 0) {
    state.lofiTrackIndex = index;
  }
}

let lofiLoopGuard = false;

function attachSpotifyControllerEvents(controller) {
  controller.addListener("ready", () => {
    state.spotifyControllerReady = true;
    setLofiStatus("ONLINE");
    renderLofiPlaybackState();
  });

  controller.addListener("playback_started", event => {
    state.lofiPlaying = true;
    state.lofiCurrentPlayingUri = event?.data?.playingURI || "";
    syncLofiTrackIndexFromUri(state.lofiCurrentPlayingUri);
    renderLofiPlaybackState();
  });
  ...
  controller.addListener("playback_update", event => {
    ...
    state.lofiPlaying = !data.isPaused && !data.isBuffering;

    if (data.playingURI) {
      state.lofiCurrentPlayingUri = data.playingURI;
      syncLofiTrackIndexFromUri(data.playingURI);
    }
    ...
    if (
      ...
      Number(data.position) >= Number(data.duration) - 700 &&
      !lofiLoopGuard
    ) {
      lofiLoopGuard = true;

      setTimeout(() => {
        state.spotifyController?.restart();
        state.spotifyController?.play();
...
function ensureSpotifyController() {
  ...
  if (!item || !els.spotifyEmbedHost) return;

  const spotifyUrl = normalizeSpotifyUrl(item.spotifyUrl, "playlist");

  if (!spotifyUrl) {
    setLofiStatus("LINK INVÁLIDO");
    return;
  }
  ...
  const IFrameAPI = window.__featherSpotifyIframeAPI;

  if (!IFrameAPI) {
    setLofiStatus("A CARREGAR...");
    return;
  }

  setLofiStatus("A LIGAR...");

  IFrameAPI.createController(
    els.spotifyEmbedHost,
    {
      width: "100%",
      height: 352,
      url: spotifyUrl
    ...
    controller => {
      ...
      attachSpotifyControllerEvents(controller);
    }
  );
}

window.addEventListener("feather-spotify-ready", () => {
...
function selectLofiPlaylist(id) {
  ...
  if (url && state.spotifyController) {
    state.spotifyController.loadEntity(url, false, 0);
    setLofiStatus("ONLINE");
  } else {
    ensureSpotifyController();
...
function renderLofiStationList() {
  ...
  for (const item of items) {
    ...
    button.classList.toggle(
      "active",
      item.id === state.selectedLofiPlaylistId
    );
    ...
    const trackCount = lofiTrackUrls(item).length;
    const meta = document.createElement("span");
    meta.textContent = trackCount
      ? `${trackCount} MÚSICAS MAPEADAS`
      : "SPOTIFY PLAYLIST";
...
function renderLofiView() {
  ...
  els.lofiStationName.textContent =
    safeText(item.name, "Feather Lofi");

  els.lofiStationDescription.textContent =
    safeText(
      item.description,
      "Playlist oficial selecionada para este arquivo."
    );

  els.lofiCampaignName.textContent =
    safeText(campaign?.title, "FEATHER STUDIOS");

  els.lofiCampaignCode.textContent =
    `${safeText(campaign?.code, "FA")} // AUDIO ARCHIVE`;
  ...
  if (background) {
    els.lofiBackdrop.style.backgroundImage =
  ...
  els.lofiControlHint.textContent = hasTracks
    ? `${tracks.length} músicas registadas // Random, Seguinte, Anterior e Loop ativos.`
    : "Player Spotify ativo. Para controlos Feather avançados, adiciona as músicas desta playlist no Admin.";
...
function renderLofiCampaignOptions() {
  ...
  const campaigns = [...state.campaigns].sort((a, b) =>
  ...
  if (current && campaigns.some(c => c.id === current)) {
    els.lofiCampaignSelect.value = current;
  }
}
...
function editLofiStation(item) {
  ...
  els.lofiTracksInput.value = lofiTrackUrls(item).join("\n");
  ...
  els.lofiManagePanel.scrollIntoView({
    behavior: "smooth",
    block: "start"
  });
}
...
async function deleteLofiStation(item) {
  ...
  if (!confirm(`Eliminar a estação "${safeText(item.name)}"?`)) {
    return;
  }

  try {
    await commitDeletes([
      doc(db, "lofiPlaylists", item.id)
    ]);

    if (els.lofiEditId.value === item.id) {
      resetLofiForm();
    }

    if (state.selectedLofiPlaylistId === item.id) {
      state.selectedLofiPlaylistId = null;
      state.lofiTrackIndex = -1;
      state.lofiTrackMode = false;
      state.lofiLoop = false;
...
function renderLofiManagement() {
  ...
  if (!items.length) {
    els.lofiManageList.innerHTML =
      `<div class="people-empty-inline">Nenhuma estação criada.</div>`;
    return;
  }
  ...
  for (const item of items) {
    ...
    const meta = document.createElement("span");
    meta.textContent =
      `${safeText(campaign?.code, "FA")} // ${tracks.length || 0} TRACKS`;
...
els.lofiPlayBtn.addEventListener("click", () => {
  if (!state.spotifyController) {
    ensureSpotifyController();
    showToast("LOFI // O player ainda está a carregar.");
    return;
  }

  state.spotifyController.togglePlay();
});

els.lofiPrevBtn.addEventListener("click", lofiPrevious);
els.lofiNextBtn.addEventListener("click", lofiNext);
els.lofiRandomBtn.addEventListener("click", lofiRandom);

els.lofiLoopBtn.addEventListener("click", () => {
  if (!lofiTrackUrls(selectedLofiPlaylist()).length) return;

  state.lofiLoop = !state.lofiLoop;
  renderLofiPlaybackState();

  showToast(
    state.lofiLoop
      ? "LOFI // LOOP ATIVO."
      : "LOFI // LOOP DESATIVADO."
  );
});

els.lofiPlaylistModeBtn.addEventListener("click", () => {
  loadLofiPlaylistEntity({autoplay:true});
});
...
els.lofiForm.addEventListener("submit", async event => {
  ...
  const spotifyUrl = normalizeSpotifyUrl(
    els.lofiSpotifyUrlInput.value,
    "playlist"
  );

  if (!campaignId || !name || !spotifyUrl) {
    els.lofiFormMessage.textContent =
      "Escolhe o RPG, dá um nome à estação e usa um link válido de playlist Spotify.";
    return;
  }

  const trackUrls = String(els.lofiTracksInput.value || "")
    .split(/\r?\n/)
    .map(line => normalizeSpotifyUrl(line, "track"))
    .filter(Boolean);
  ...
  try {
    ...
    if (els.lofiEditId.value) {
      await updateDoc(
        doc(db, "lofiPlaylists", els.lofiEditId.value),
        data
      );
    ...
    els.lofiFormMessage.textContent =
      error.message || "Não foi possível guardar a estação.";
  }
});
...
function startPublicListeners() {
  ...
  onSnapshot(
    ...
    snapshot => {
      ...
      if (
        previousSelected &&
        previousSelected !== state.selectedLofiPlaylistId
      ) {
        state.lofiTrackIndex = -1;
        state.lofiTrackMode = false;
        state.lofiLoop = false;
