...
function spotifySleep(ms) {
  return new Promise(resolve => setTimeout(resolve, ms));
}

async function spotifyWaitForDevice(deviceId, {attempts = 18, delay = 450} = {}) {
  let lastDevices = [];

  for (let attempt = 0; attempt < attempts; attempt += 1) {
    try {
      const result = await spotifyApiRetry("/me/player/devices");
      const devices = Array.isArray(result?.devices) ? result.devices : [];
      lastDevices = devices;

      // 1) Melhor caso: o ID devolvido pelo SDK coincide com o Spotify Connect.
      let device = devices.find(item => item.id === deviceId);
      if (device) return device;

      // 2) O SDK pode reconectar e o Spotify Connect devolver o mesmo player
      // com um ID diferente. Nesse caso, encontramos pelo nome e adotamos
      // imediatamente o ID que a Web API considera válido.
      const featherCandidates = devices.filter(item =>
        String(item?.name || "").trim().toLowerCase() === "feather lofi" &&
        item?.is_restricted !== true
      );

      device =
        featherCandidates.find(item => item.is_active === true) ||
        featherCandidates[0] ||
        null;

      if (device?.id) {
        if (device.id !== state.spotifyDeviceId) {
          console.info(
            "[FEATHER] Spotify Connect devolveu um ID diferente para Feather Lofi.",
            {sdkDeviceId: state.spotifyDeviceId, connectDeviceId: device.id}
          );
        }

        state.spotifyDeviceId = device.id;
        return device;
      }

      // Diagnóstico útil sem expor tokens.
      if (attempt === 0 || attempt === attempts - 1) {
        console.info(
          "[FEATHER] Devices Spotify Connect visíveis:",
          devices.map(item => ({
            name: item.name,
            id: item.id,
      ...
      console.warn("[FEATHER] Spotify device check:", error);
    }

    await spotifySleep(delay);
  }

  console.warn(
    "[FEATHER] Feather Lofi não apareceu em /me/player/devices.",
    lastDevices.map(item => item?.name || "SEM NOME")
  );

  return null;
}

async function spotifyWaitUntilActive(deviceId, {attempts = 14, delay = 350} = {}) {
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    try {
      const playback = await spotifyApiRetry("/me/player");
      if (playback?.device?.id === deviceId && playback.device.is_active !== false) {
        return true;
      }
    } catch (error) {
      console.warn("[FEATHER] Spotify active-device check:", error);
    }
...
async function spotifyPrepareFeatherDevice() {
  if (!state.spotifyPlayer || !state.spotifyDeviceId) return false;

  // O Web Playback SDK pode emitir `ready` alguns instantes antes de o mesmo
  // device aparecer na Web API. Esperamos explicitamente essa sincronização.
  els.lofiSpotifyStatus.textContent = "A REGISTAR DEVICE...";

  let device = await spotifyWaitForDevice(state.spotifyDeviceId);

  if (!device) {
    // Uma segunda tentativa de connect costuma resolver devices que ficaram
    // prontos no SDK mas ainda não chegaram ao Spotify Connect Web API.
    try {
      await state.spotifyPlayer.connect();
    } catch {}

    device = await spotifyWaitForDevice(
      state.spotifyDeviceId,
      {attempts: 10, delay: 450}
    );
  }

  if (!device) {
    throw new Error("O FEATHER LOFI NÃO APARECEU NA LISTA DE DEVICES DO SPOTIFY CONNECT.");
  }

  if (device.is_restricted) {
    throw new Error("O SPOTIFY MARCOU ESTE DEVICE COMO RESTRITO.");
  }

  // Usa sempre o ID confirmado pelo endpoint /me/player/devices. Ele pode ser
  // diferente do ID que o SDK entregou no evento ready após uma reconexão.
  if (device.id) {
    state.spotifyDeviceId = device.id;
  }

  // Tornamos primeiro o Feather Lofi no device ativo. Só enviamos o próximo
  // comando depois de confirmar via /me/player que a transferência terminou.
  els.lofiSpotifyStatus.textContent = "A ATIVAR DEVICE...";
  ...
  const active = await spotifyWaitUntilActive(state.spotifyDeviceId);

  if (!active) {
    throw new Error("NÃO FOI POSSÍVEL ATIVAR O DEVICE FEATHER.");
  }
...
async function spotifyStartSelectedPlaylist() {
  ...
  try {
    // Tem de acontecer como consequência direta do clique para desbloquear
    // áudio no browser antes de prepararmos o Spotify Connect device.
    await state.spotifyPlayer.activateElement?.();

    await spotifyPrepareFeatherDevice();

    els.lofiSpotifyStatus.textContent = "A CARREGAR PLAYLIST...";

    // Como acabámos de confirmar que o Feather é o device ativo, omitimos
    // device_id aqui. Isto evita o erro intermitente `Device not found` que
    // ocorre quando o id do SDK ainda está a propagar pela Player Web API.
    ...
    await spotifySleep(650);

    let current = await state.spotifyPlayer.getCurrentState();

    // Há contas/browsers em que o estado local demora mais uma ronda.
    if (!current) {
      await spotifySleep(650);
