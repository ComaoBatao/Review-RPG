...
const els = {
  ...
  characterBannerInput: $("#characterBannerInput"),
...
function openCharacterModal(characterId) {
  ...
  const campaign = state.campaigns.find(c => c.id === character.campaignId);
  const episodes = characterEpisodes(character.id).sort((a,b) => Number(a.number||0)-Number(b.number||0));
  const cover = els.characterModalHero.querySelector(".character-modal-cover");
  const banner = safeHttpUrl(character.bannerUrl);
  cover.style.backgroundImage = banner ? `url("${banner.replace(/\"/g,'%22')}")` : "linear-gradient(135deg,#372419,#11100e)";
  const image = safeHttpUrl(character.imageUrl);
  els.characterModalPortrait.innerHTML = image ? `<img src="${image}" alt="">` : `<span>${safeText(character.name,"?").charAt(0)}</span>`;
  els.characterModalCode.textContent = `CHARACTER FILE // ${safeText(campaign?.code,"ARQ")}`;
  els.characterModalName.textContent = safeText(character.name);
  els.characterModalDescription.textContent = safeText(character.description, "Sem descrição registada.");
  els.characterModalMeta.innerHTML = `
    <div><span>PLAYER</span><strong>${safeText(player?.name,"—")}</strong></div>
    <div><span>ARQUIVO</span><strong>${safeText(campaign?.title,"—")}</strong></div>
    <div><span>EPISÓDIOS</span><strong>${episodes.length}</strong></div>`;
  els.characterModalEpisodes.innerHTML = episodes.length ? `<span>APARIÇÕES</span>` : `<div class="people-empty-inline">Ainda sem aparições registadas.</div>`;
  for (const episode of episodes) {
    const btn = document.createElement("button");
    btn.type = "button";
    btn.textContent = `EP${String(episode.number||0).padStart(2,"0")} — ${safeText(episode.title)}`;
    btn.addEventListener("click", () => { closeModal(els.characterModal); openEpisodeFromPeople(episode); });
...
function editCharacter(character) {
  renderPeopleAdminOptions(); els.characterEditId.value = character.id; els.characterNameInput.value = character.name || ""; els.characterPlayerSelect.value = character.playerId || ""; els.characterCampaignSelect.value = character.campaignId || ""; els.characterImageInput.value = character.imageUrl || ""; els.characterBannerInput.value = character.bannerUrl || ""; els.characterDescriptionInput.value = character.description || ""; els.characterFormTitle.textContent = "Editar Personagem"; els.characterSaveBtn.textContent = "GUARDAR PERSONAGEM"; els.characterCancelEditBtn.classList.remove("hidden");
}
...
els.characterForm.addEventListener("submit", async event => {
  ...
  const data = {name, playerId, campaignId, imageUrl:els.characterImageInput.value.trim(), bannerUrl:els.characterBannerInput.value.trim(), description:els.characterDescriptionInput.value.trim().slice(0,700), updatedAt:serverTimestamp(), updatedBy:state.user.uid};
  try {
