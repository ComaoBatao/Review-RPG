...
async function spotifyWaitForDevice(deviceId, {attempts = 14, delay = 350} = {}) {
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    try {
      const result = await spotifyApiRetry("/me/player/devices");
      const device = (result?.devices || []).find(item => item.id === deviceId);
...
async function spotifyPrepareFeatherDevice() {
  ...
  if (!device) {
    throw new Error("O DEVICE FEATHER NÃO APARECEU NO SPOTIFY CONNECT.");
  }
