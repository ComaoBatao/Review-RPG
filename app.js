...
els.campaignForm.addEventListener("submit", async (event) => {
  ...
  if (!code || !title) {
    els.campaignMessage.textContent = "Preenche o código e o nome do arquivo.";
    return;
  }
  ...
  try {
    ...
    const ref = await addDoc(collection(db, "campaigns"), {
      ...
      coverUrl: els.campaignCoverUrl.value.trim(),
      createdAt: serverTimestamp(),
    ...
    els.campaignMessage.textContent = error.message || "Não foi possível criar o arquivo.";
  }
});
...
els.episodeForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    await addDoc(collection(db, "episodes"), {
      ...
      imageUrl: els.episodeImageUrl.value.trim(),
    ...
    els.episodeMessage.textContent = error.message || "Não foi possível publicar o episódio.";
  }
});
...
els.editEpisodeForm.addEventListener("submit", async event => {
  ...
  try {
    ...
    await updateDoc(doc(db, "episodes", episodeId), {
      ...
      imageUrl: els.editEpisodeImageUrl.value.trim(),
      updatedAt: serverTimestamp(),
    ...
    els.editEpisodeMessage.textContent = error.message || "Não foi possível guardar as alterações.";
  }
});
